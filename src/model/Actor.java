...
public class Actor {
    ...
    protected int x,y,type,orient, width, height, speed, runBomb;
	protected Image img;
	public void drawActor(Graphics2D g2d) {
	};
	
	public void changeOrient(int orient) {
		this.orient = orient; // thay đổi hướng
	}
	...
	public void setRunBomb(int runBomb) {
		this.runBomb = runBomb;  // thiết lập giá trị cho runbomb
	}
	...
	public void setSpeed(int speed) {
		if (speed < 1) {
			return;    // Không cho phép gia trị nhỏ hơn 1
		}
		this.speed = speed;
	}
    ...
    public boolean move(int count, ArrayList<Bomb> arrBomb, ArrayList<Box> arrBox) {
		...
		case LEFT:
			...
			for (int i = 0; i < arrBomb.size(); i++) {
				if (arrBomb.get(i).BombvsActor(this) == 1) {
					x = x + 1;
					return false;
				}
			}
